...
- Once on that new issue branch, create and amend commits without asking until an external review system can attach comments or findings to the published history. Direct maintainer-to-agent feedback does not freeze history. On an existing, shared, or unrelated branch, obtain permission before committing.
...
## Preserve externally reviewed history

Once an external review system has attached human comments or automated findings, such as SonarCloud results, treat every commit in the reviewed revision as frozen. Direct feedback from the maintainer to the agent is not external review; local history may still be rewritten or force-pushed when the maintainer permits it.
