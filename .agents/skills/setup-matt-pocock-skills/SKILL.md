...
Implement issues on a new, dedicated issue branch using small, reasoned commits. Agents may create commits there without asking. Review and, when necessary, rewrite the local commit sequence before first publication. Preserve commits once an external review system can attach comments or findings to them, and add review fixes as follow-up commits. Direct maintainer-to-agent feedback does not freeze history. See `docs/agents/commits.md`.
```
